...
(setq org-agenda-custom-commands
      (quote (
		...
		(todo "WORKING|NEXT|TODO|BLOCKED"
                      ((org-agenda-overriding-header (hs/org-style-sub-header "Scheduled"))
		...
		(tags-todo "+TODO+DEADLINE<=\"<2022-12-31>\""
                      ((org-agenda-overriding-header (hs/org-style-sub-header "Deadlines"))
		       (org-agenda-compact-blocks t)
		       (org-agenda-skip-function '(org-agenda-skip-subtree-if 'notdeadline))
		       (org-agenda-prefix-format " %-20(let ((deadline (org-get-deadline-time (point)))) (if deadline (format-time-string \"%d %b %Y\" deadline) \"<sub>\"))  ")
		       (org-agenda-sorting-strategy '(scheduled-up time-up))))
		(todo "WORKING|NEXT|TODO|BLOCKED"
                      ((org-agenda-overriding-header (hs/org-style-sub-header "Unscheduled"))
		       ;;(org-agenda-prefix-format " %-20(let ((scheduled (org-get-scheduled-time (point)))) (if scheduled (format-time-string \"%d %b '%y\" scheduled) \"<sub>\")):  ")
		       (org-agenda-prefix-format '((todo . "%i   %-20c")))
		       (org-agenda-compact-blocks t)
		       (org-agenda-skip-function '(org-agenda-skip-subtree-if 'scheduled 'deadline))
		...
		(tags "+project"
	      ...
	      ("u" "Items with no assigned projects"
	       (
		(tags-todo "-project-meeting/WORKING|NEXT|TODO|BLOCKED"
                      ((org-agenda-overriding-header (hs/org-style-sub-header "Projectize these items:"))
		       (org-agenda-compact-blocks t)
	      ...
	      ("r" "Read / Watch / Listen and Learn"
	       (
		(tags "+read"
                      ((org-agenda-overriding-header
			(hs/org-style-header "Read / Watch / Listen and Learn"))
		       (org-agenda-compact-blocks t)
                       (org-tags-match-list-sublevels nil)
...
*** Load org-roam using Use-Package
    I have disabled this since for some reason enabling org-roam using
    use-package is resulting in org-roam-ui complaining that i'm using
    old version of org-roam. After a bit of debugging, I found that
    it's some symbol called org citations or something (search for
    citations in org-roam-db.el file) was missing in my
    org-roam-db.el. So for now, I am including org-roam directly from
    git in my private packages folder. Until that gets resolved, this
...
(use-package org-roam
   :ensure t
   :custom
   (org-roam-directory (file-truename "~/mindmap/"))
...
** [LIB] Org Mode Library Functions
   ...
   #+BEGIN_SRC emacs-lisp
    ...
    (defun hs/org-roam-goto-todays-journal()
      (find-file )
      )
   #+END_SRC
